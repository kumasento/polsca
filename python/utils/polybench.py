...
class PbFlow:
    ...
    def copy_design_from_phism_to_tb(self, try_fix=True):
        ...
        # destination_name : the name of the updated design file
        # top_func : the name of the top-level design
        def update_interface(source_name, tb_name, destination_name, top_func):
            source_file = open(source_name)
            tb_file = open(tb_name)
            ...
            destination_file.write(source_line)
            source_line = source_file.readline()
            while "end;" not in source_line:
                top_port_list.append(source_line)
                source_line = source_file.readline()
            ...
            tb_line = tb_file.readline()
            tb_line = tb_file.readline()
            while "end component;" not in tb_line:
            ...
            destination_file.write("-- matched ports begin\n")
            for line in tb_port_list:
                destination_file.write(line)
            destination_file.write("-- matched ports end\n")
            destination_file.write(source_line)

            source_line = source_file.readline()
            while "begin" not in source_line:
                destination_file.write(source_line)
                source_line = source_file.readline()
            destination_file.write(source_line)

            # In case floating write-enable ports affect the results, they are set to zero
    ...
    def run_cosim(self):
        ...
        src_file = self.cur_file
        base_dir = os.path.dirname(src_file)
        sim_dir = os.path.dirname(src_file, "tb", "solution1", "sim", "vhdl")
